//
// Created by guillermo on 6/21/18.
//
...
tFitness realea::YYOPMemeDif::evalSol(tChromosomeReal &orig) {
...
realea::tChromosomeReal realea::YYOPMemeDif::solToDim(tChromosomeReal & s){
...
realea::tChromosomeReal realea::YYOPMemeDif::solToOne(tChromosomeReal & s){
    tChromosomeReal a(s.size());
    std::transform(s.begin(),s.end(),a.begin(),
...
tFitness realea::YYOPMemeDif::splitStage(tChromosomeReal & s, tReal d){
...
unsigned realea::YYOPMemeDif::lsStage(tChromosomeReal & sol, tFitness & fitness, unsigned iters){
    /*
     * Local Search
     * */
    ILocalSearch *ls;
...
unsigned realea::YYOPMemeDif::difStage(vector<tChromosomeReal> &arc, vector<tFitness> & fitness) {
    ...
    for(int i=0; i<POP_SIZE; i++){
        ...
        //cout << ";" << endl;
        int     r1 = m_random->randint(0,(SOL_SIZE-1)/2)*2,
                r2 = m_random->randint(0,(SOL_SIZE-1)/2)*2,
                r3 = m_random->randint(0,(SOL_SIZE-1)/2)*2 + 1;
        while(r2 == r1 || r2 == i) r2 = m_random->randint(0,SOL_SIZE-1);
        while(r3 == r2 || r3 == r1 || r3 == i) r3 = m_random->randint(0,SOL_SIZE-1);
        //Mutate
        //For gene
        for(int j=0; j<SOL_SIZE; j++){
            ...
            if(r<CR){
                ...
                mut[i][j] = arc[i][r1] + F*(arc[i][r2]-arc[i][r3]);
                //cout << "3" << endl;
                //Check bound
                if (mut[i][j] > 1)
                    mut[i][j] = 1;
...
unsigned realea::YYOPMemeDif::apply(tChromosomeReal &sol, tFitness &fitness, unsigned itera) {
    ...
    p1 = solToDim(p1);
    iters -= ls->apply(ls_options,p1,eval1,LS_ITERS/2);
    p1 = solToOne(p1);
    ...
    while((iters-added-LS_ITERS) > 0 || i != 0){ //After archive stage
        //while(false){
        //Check fittest
        if(eval2 < eval1){
            swp();
