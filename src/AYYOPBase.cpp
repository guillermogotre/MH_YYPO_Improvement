...
tFitness realea::AYYOPBase::evalSol(tChromosomeReal &orig) {
...
tFitness realea::AYYOPBase::splitStage(tChromosomeReal & s, tReal d){
    const unsigned D = s.size();
    const double SPLIT_PROB = 1-(D/(D+5));
...
unsigned realea::AYYOPBase::apply(tChromosomeReal &sol, tFitness &fitness, unsigned itera) {

    const unsigned I_MIN = 2, I_MAX = 4, I = m_random->randint(I_MIN,I_MAX);
    ...
    tReal alpha = 10;
    tReal d1 = 0.5, d2 = 0.5;
    tChromosomeReal p1 = sol, p2(sol.size());

    for(unsigned i=0; i<sol.size(); i++){
        p2[i] = m_random->randreal(minV,maxV);
    }
    ...
    //Aux functions
    auto swp = [&](){p1.swap(p2);swap(eval1,eval2),swap(d1,d2);};
